...
The distribution version requires a licensing server to permit users to log in, which we've now taken down. To compensate, we've removed the registration module, allowing people to use the product for free (if they wish). After upgrading a number of the backend scanning routines, we can now support Windows 7, 8, 10 & 11..
