...
namespace Cll
{
	...
	public class xmlRegistry
	{
		...
		[DllImport("advapi32.dll", EntryPoint="RegEnumKeyEx")] public static extern int RegEnumKeyExA(int hKey, int dwIndex, ref byte lpName, ref int lpcbName, int lpReserved, string lpClass, ref int lpcbClass, ref FILETIME lpftLastWriteTime);
		...
		public static bool keyExists(string strMainKey, string strPath)
		{
			...
			RegistryKey reg = openKey(strMainKey,strPath);
		...
		public static RegistryKey openKey(string strMainKey, string strPath)
		{
		...
		void LoadAsXml_SetValue(int hKey, string strName, string strValue, string strType)
		{
			...
			//MessageBox.Show (string.Format("couldn't set value {0}", strName ), "Office Recovery Manager", 
			//	MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
		}
